...
[Ilija Lichkovski](https://x.com/carnot_cyclist) makes a closely related distinction in a thread on [defining continual learning for LLMs](https://x.com/carnot_cyclist/status/2041479655035679163): growing harness-side memory (skill trees, banks of notes) can show diminishing returns or even hurt when the object under the harness is still a fixed parametric model, whereas updating weights changes how much *useful* computation can live in a single forward pass and is where compounding is supposed to show up—if you can solve the harder parametric continual-learning problem.
...
The loop matters. The signal matters. The *language* and *structure* of state persistence matters. But these things do not help monotonically, and they do not help equally across tasks. Sometimes the clever mechanism wins. Sometimes the plain chat loop wins. Sometimes the judge gets fooled. Sometimes the benchmark metric disagrees. And sometimes, which is perhaps the healthiest outcome for a study like this, the experiment tells you that the story you wanted was a little too clean.
