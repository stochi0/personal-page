export const metadata = {
  title: "Weight-Tying: The Small, Gentle Read–Write Symmetry in Language Models",
...
import forwardPassPipeline from "./forward-pass-tokens-to-logits.png";
import projectionToLogits from "./projection-H-Et-to-logits.png";
import gradientFlowIntoE from "./gradient-flow-into-embedding-E.png";
import outerProductUpdate from "./outer-product-update-per-position.png";
...
<img src={forwardPassPipeline.src} alt="Forward pass pipeline: raw text → tokenizer → embeddings + position → transformer → logits (H E^T + b) → softmax → sample" />
...
<img src={projectionToLogits.src} alt="Projection matrix multiplication: hidden states H × E^T → logits Z (Z = H E^T + b)" />
...
<img src={gradientFlowIntoE.src} alt="Gradient flow into embedding matrix E: input-side (sparse) and output-side (dense) contributions sum to ∂ℓ/∂E" />
...
<img src={outerProductUpdate.src} alt="Outer product update per position: residuals Δ and hidden state h produce the output-side update to E" />
